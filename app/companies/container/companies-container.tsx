...
export function CompaniesContainer() {
  ...
  // 2. Initialize the hook next to your delete mutation hook line
  const generateNdaMutation = useGenerateNdaMutation();

  async function handleGenerateNDA(company: Company) {
    const toastId = toast.loading(`Generating NDA for ${company.name}...`);
    
    // 3. Fire the mutation using the authorized apiService wrapper channel
    generateNdaMutation.mutate(company.id, {
      onSuccess: (fileBlob) => {
        // Build a memory pipeline target block for streaming binary response types
        ...
        toast.success("NDA downloaded successfully!", { id: toastId });
      },
      onError: (err) => {
        console.error("NDA Hook Download Error:", err);
        toast.error("Could not generate NDA. Please verify admin system logs.", { id: toastId });
      }
    });
  }
                  ...
                  <div className="flex justify-end">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        {/* Changed to clean vertical ellipsis ghost button */}
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onClick={() => handleGenerateNDA(company)}>
